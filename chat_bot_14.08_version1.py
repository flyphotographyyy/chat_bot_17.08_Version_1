def _turnover(weights: pd.DataFrame) -> float:
    """Annualized portfolio turnover using 0.5*sum|Δw| per rebalance."""
    if weights is None or weights.empty:
        return 0.0
    w = weights.fillna(0.0).copy()
    delta = w.diff().abs().sum(axis=1).dropna()
    if delta.empty:
        return 0.0
    d = w.index.to_series().diff().dropna().dt.days
    per_year = max(1, int(round(365 / (d.mean() if len(d) else 30))))
    return float((0.5 * delta.mean()) * per_year)

def _ann_return(daily: pd.Series) -> float:
    if daily is None or len(daily) == 0:
        return 0.0
    # geometric annualization
    total = float((1.0 + pd.Series(daily).fillna(0.0)).prod())
    n = len(daily)
    if n == 0: return 0.0
...
def portfolio_walkforward_backtest(
    ...
    cost_bps: int = 10,
    slip_bps: int = 5,
    min_hold_days: int = 7,   # kept for signature compatibility; enforced implicitly by monthly/weekly
) -> Dict:

    # Vectorized walk-forward with weekly/monthly rebalance, equal-weight top-K, cost on turnover,
    # vol targeting (scale <= 1), bull/bear regime gating optional via SPY momentum.
    try:
        # Ensure a cash-like fallback exists
        cash_candidates = ["SHY","BIL","SGOV","GBIL"]
        base_tickers = list(dict.fromkeys(tickers))
        cash = next((c for c in cash_candidates if c in base_tickers), None)
        if cash is None:
            base_tickers = base_tickers + ["SHY"]
            cash = "SHY"

        start = (dt.date.today() - dt.timedelta(days=365*10)).isoformat()
        end = (dt.date.today() + dt.timedelta(days=1)).isoformat()
        prices = yf.download(base_tickers, start=start, end=end, auto_adjust=True, progress=False)["Close"]
    ...
    if prices.empty or prices.shape[0] < 252:
        return {"oos_trades": 0, "oos_equity": 1.0, "oos_CAGR": 0.0, "oos_maxDD": 0.0, "oos_turnover": 0.0, "oos_sharpe": 0.0}

    # Compute indicators on price panel
    rets = prices.pct_change().fillna(0.0)
    mom_lb = max(84, int(round(test_m*22*1.5)))  # ~4-6m
    mom = (prices / prices.shift(mom_lb) - 1.0)
    sma200 = prices.rolling(200).mean()
    vol20 = rets.rolling(20).std() * np.sqrt(252)
    spy = prices.get("SPY", prices.iloc[:,0])
    spy_mom = float((spy.iloc[-1] / spy.shift(126).iloc[-1]) - 1.0) if len(spy) > 126 else 0.0

    # Rebalance dates
    if rebalance.upper().startswith("W"):
        rb = prices.resample(rebalance).last().index
    else:
        rb = prices.resample("M").last().index  # monthly default

    # Walk-forward over rolling train/test windows by index slices
    ...
    for seg_start in range(start_i, len(all_idx)-1, step):
        ...
        seg_idx = all_idx[seg_start:seg_end+1]

        # Rebalance points inside this segment
        rb_seg = [d for d in rb if seg_idx[0] <= d <= seg_idx[-1]]
        if not rb_seg:
            continue

        for i, d in enumerate(rb_seg):
            # Selection filter (regime: only demand SPY momentum >= 0; don't fully gate = reduces cash drag)
            # Compute scores at date d using available data up to d
            px_d = prices.loc[:d]
            if px_d.shape[0] < 200:
                continue
            row_prices = px_d.iloc[-1]
            row_sma200 = sma200.loc[d] if d in sma200.index else sma200.iloc[-1]
            row_mom = mom.loc[d] if d in mom.index else mom.iloc[-1]
            row_vol = vol20.loc[d] if d in vol20.index else vol20.iloc[-1]

            # Candidates
            candidates = (row_prices > row_sma200) & (row_vol <= 0.60)
            # Score: momentum + trend bonus
            scores = (0.7*row_mom).fillna(-1.0) + (0.3*((row_prices/row_sma200) - 1.0)).fillna(0.0)
            scores = scores[candidates.fillna(False)]
            # keep cash available
            if cash in scores.index:
                scores = scores.drop(cash, errors="ignore")

            # Pick Top-K
            sel = list(scores.sort_values(ascending=False).head(top_k).index)
            if len(sel) == 0:
                # fallback -> cash
                w = pd.Series(0.0, index=prices.columns); w[cash] = 1.0
            else:
                w = pd.Series(0.0, index=prices.columns)
                w[sel] = 1.0/len(sel)

            # Vol targeting (scale <= 1)
            # Estimate next-period daily vol from last 20d
            est_vol = float(rets[sel].rolling(20).std().iloc[-1].mean()) if sel else 0.0
            if np.isnan(est_vol) or est_vol <= 0:
                scale = 1.0
            else:
                annual_vol = est_vol * np.sqrt(252)
                target = 0.10 if risk_profile=='conservative' else (0.25 if risk_profile=='aggressive' else 0.17)
                scale = float(np.clip(target / max(1e-9, annual_vol), 0.4, 1.0))
            w *= scale

            # Compute turnover and apply cost ONCE at the start of holding period
            delta_w = (w - prev_w).abs().sum()
            turnover = 0.5 * float(delta_w)  # half-sum convention
            cost = turnover * (cost_bps + slip_bps) / 10000.0

            # Compute returns for [d, next_d)
            if i == len(rb_seg)-1:
                next_d = seg_idx[-1]
            else:
                next_d = rb_seg[i+1]
            win = prices.loc[d:next_d]
            pr = win.pct_change().fillna(0.0)
            port_r = (pr * w).sum(axis=1)
            if len(port_r) > 0:
                # apply cost at the first day (if same-day only one record, subtract there)
                first_idx = port_r.index[0]
                port_r.loc[first_idx] = port_r.loc[first_idx] - cost
                oos_daily.append(port_r)
                weights_records.append((d, w.copy()))
    ...
    cum = (1.0 + oos).cumprod()
    weights_df = pd.DataFrame({d: w for d, w in weights_records}).T if len(weights_records) else pd.DataFrame()

    mets = {
        "oos_CAGR": float(_ann_return(oos)),
        "oos_maxDD": float(abs((cum/cum.cummax()-1.0).min())) if len(cum) else 0.0,
        "oos_sharpe": float((oos.mean()/oos.std())*np.sqrt(252)) if oos.std()>0 else 0.0,
        "oos_turnover": float(_turnover(weights_df)),
        "oos_trades": int((weights_df.diff().abs().sum(axis=1)>0).sum()) if not weights_df.empty else 0,
        "oos_equity": float(cum.iloc[-1]) if len(cum) else 1.0,
    }
    return mets
